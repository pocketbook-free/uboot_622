...
#define STATE_OF_CHARGE_THRESHHOLD 4
#define BATTERY_VOLTAGE_THRESHHOLD 3640
...
void check_battery(void)
{
	...
	while (1) {
		...
		adc1 = MC13892_ADC1_ADEN | 
		       MC13892_ADC1_ADCCAL | 
		       MC13892_ADC1_ADTRIGIGN | 
		       MC13892_ADC1_ASC | 
		...
		if (soc > STATE_OF_CHARGE_THRESHHOLD || volt_bat > BATTERY_VOLTAGE_THRESHHOLD)
			break;
		...
		if (IsChargerPresence(val)) {
			...
			debug("change charge current to 400mA ...\n");
...
vidinfo_t panel_info = {	//accordding the E60_V220 firmwire modify the paramer
	.vl_refresh = 60,	//60
	.vl_col = 800,
	.vl_row = 600,
	.vl_pixclock = 26000000,
	.vl_left_margin = 4,
	.vl_right_margin = 93,
	.vl_upper_margin = 4,
	.vl_lower_margin = 8,
...
int setup_waveform_file()
{
	...
	unsigned char run_type;
	const char *run_type_name = "UNK";
	char *s = NULL;
	struct mmc *mmc = find_mmc_device(mmc_dev);
	uint blk_start, blk_cnt, n;
	...
	debug("mmc->read_bl_len is %d\n", mmc->read_bl_len);
	blk_start = ALIGN(offset, mmc->read_bl_len) / mmc->read_bl_len;
	debug("blk_start is %d\n", blk_start);
	blk_cnt   = ALIGN(size, mmc->read_bl_len) / mmc->read_bl_len;
	debug("blk_cnt is %d\n", blk_cnt);
	n = mmc->block_dev.block_read(mmc_dev, blk_start,
	                              blk_cnt, (u_char *)addr);
	...
	if (*(waveform_head + 0x14) != 0x3c) {
		printf("Fatal error: Can't find waveform!\n");
		return -1;
	}

	waveform_size = *((uint *)waveform_head + 1);
	debug("Waveform size is %d Bytes\n", waveform_size);

	run_type = *((unsigned char *)waveform_head + 12);
	if (run_type == 0x05)
		run_type_name = "C";
	else if (run_type == 0x07)
		run_type_name = "E";

	fpl_lot_num = *((unsigned short *)waveform_head + 7);
	printf("fpl_lot_num is %d\n", fpl_lot_num);

	printf("WF run type 0x%x %s%d\n", (unsigned)run_type, run_type_name, fpl_lot_num);
...
struct reco_envs supported_reco_envs[BOOT_DEV_NUM] = {
	...
	},
	{
		...
		.args = CONFIG_ANDROID_RECOVERY_BOOTARGS_MMC,//dev_num == 6/7 can be excute the recovery mode
	},
	{
		.cmd = CONFIG_ANDROID_RECOVERY_BOOTCMD_MMC,
