...
int drv_lcd_init (void)
{
	...
	if(lcd_init (lcd_base) != 0)		/* LCD initialization */
		return 0;
#if 0
...
#else
	lcd_setfgcolor (CONSOLE_COLOR_WHITE);	//configure front color white
...
//	printf ("[LCD] Initializing LCD frambuffer at %p\n", lcdbase);
	...
	if(lcd_ctrl_init (lcdbase)!=0)
		return -1;

	lcd_is_enabled = 1;

	if(lcd_clear (NULL, 1, 1, NULL) == 0){/* dummy args */
		lcd_enable ();
		lcd_disable();
		lcd_panel_disable();
	}
...
//	printf("%s****************\n",__func__);
	...
	if (do_splash && (s = getenv("splashimage")) != NULL) {
		...
		{
			printf("lcd_display_bitmap failed.\n");
			return NULL;
		}
	}
...
/* an interface command to debug the logo show under uboot*/
U_BOOT_CMD(
		show_logo, 1, 1, drv_lcd_init,
		"show_logo:	show logo in uboot\n",
		"show_logo under uboot\n"
		);
/************************************************************************/
