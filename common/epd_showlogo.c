...
static int eink_display_bitmap(ulong bmp_image, int x, int y)
{
	...
	if(width != 800 || height != 600){
		printf("Error:picture format %d*%d, program support 800*600 8bit bmp picture\n",width,height);
		return 1;
	}
...
static void *map_logo (void)
{
	...
	debug("do_splash %d env splashimage %0x\n", do_splash, simple_strtol(getenv("splashimage"),NULL,0));
