...
//	#define CONFIG_WAVEFORM_FILE_SIZE		0xF0000	
	...
	#define CONFIG_SPLASH_IMG_OFFSET 	0x80400//this is refer the ANDROID_LOGO_OFFSET defined in the fastboot.c
 ...
 * logo - 0x080000 ~ 0xBFFFF byte
 * logo2 - 0xc0000 ~ 0xFFFFF byte* 
 * kernel - 0x100000 ~ 0x3FFFFF byte
 * waveform - 0x400000 ~ 0x8FFFFF byte
 * root filesystem - /dev/mmcblk0p2 or /dev/mmcblk1p1
 */
/*IMX50 RDP*/
...
#define ANDROID_LOGO_OFFSET	   0x80400
#define ANDROID_LOGO_SIZE	    0x100000
#define ANDROID_LOGO2_OFFSET	    0x180400
#define ANDROID_LOGO2_SIZE	    0x100000
#define ANDROID_KERNEL_OFFSET	    0x280400
#define ANDROID_KERNEL_SIZE	    0x300000
#define ANDROID_WAVEFORM_OFFSET	    0x580600
#define ANDROID_WAVEFORM_SIZE	    0x300000
...
#define CONFIG_DIAG_FLAG 	"pocketbook "
//#define CONFIG_BOOT_DIAG	"mmc read 0 0x70805000 0x200 1"
//#define BOOTARGS_DIAG	"setenv bootargs console=ttymxc0,115200 fastboot_dev=mmc0 root=/dev/mmcblk0p2 vcom=${vcom} rootwait"
...
//#define BOOTCMD_CRAMFS	"mmc read 0 0x70800040 0x1402 0x1800; mmc read 0 0x72000000 0x4402 0xc800; bootm 0x70800040"
//#define BOOTENV_UPDATE	"vcom read; setenv bootargs root=/dev/ram0 rootfstype=cramfs rw rootdelay=2 ramdisk=32768 initrd=0x72000000,32M init=/linuxrc console=ttymxc0,115200 vcom=${vcom} rootwait;mmc read 0 0x70800040 0x1402 0x1800; mmc read 0 0x72000000 0x4402 0xc800; bootm 0x70800040"
						
#define _UBOOT_
#define CONFIG_OCOTP    /*for burn ROM*/
#define CONFIG_VCOM    // for vcom
#define CONFIG_HWCONFIG	// for hwconfig
#define CONFIG_WAVEFORM  //for waveform
#define CONFIG_SHARE_REGION	 //for recovery
...
#define CONFIG_ANDROID_APP_PARTITION_MMC 2
...
#define	CONFIG_EXTRA_ENV_SETTINGS					\
		...
		"splashpos=m,m\0"	\
		\
		"bootargs_sd=setenv bootargs console=ttymxc0,115200 ip=none root=/dev/mmcblk0p2 vcom=${vcom} rootwait\0" \
		"bootargs_nand=setenv bootargs console=ttymxc0,115200 ip=none root=/dev/mmcblk1p1 vcom=${vcom} rootwait\0" \
		\
		...
		\
        "fatload_kernel_sd=fatload mmc 0:1 ${kernel_addr} uImage; mmc write 0 ${kernel_addr} 0x1402 0x1800\0" \
...
#define CONFIG_SYS_PROMPT		"EP7 U-Boot> "
 ...
 * GPMI Nand Configs
 */
//#define CONFIG_CMD_NAND   //modify no nand device
...
//	#define CONFIG_ENV_OFFSET	(768 * 1024)
	#define CONFIG_ENV_OFFSET	(320 * 1024)   //modify for new partition
